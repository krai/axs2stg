...
class Coco_stageai(dataset.Dataset):
    def __init__(self, data_path, transform=None):
        self.captions_df = pd.read_csv(os.path.join(data_path,"captions", "captions.tsv"), sep="\t")
        self.img_dir = os.path.abspath(os.path.join(data_path, "validation", "data"))
    ...
    def get_img_path(self, idx):
        """Get img path"""
        file_name = self.captions_df.loc[idx, "file_name"]
    ...
    def get_item(self, id):
        return dict(self.captions_df.loc[id])
...
class PostProcessCoco:
    ...
    def __call__(self, results, ids, expected=None, result_dict=None):
        processed = [
            (t * 255).round().to(self.dtype).permute(1, 2, 0).cpu().numpy()
            for t in results
        ]
        with self.lock:
